    ...
    useEffect(() => {
        ...
        const verify = async () => {
            try {
                ...
                setMessage(getErrorMessage(err, 'Verification failed. The token may be expired or invalid.'));
                setStatus('error');
            }
        };
    ...
    if (!token) {
        return (
            <div className="glass-card text-center p-8">
                ...
                <p className="text-gray-400 mb-6">Please click on the verification link from your email.</p>
        ...
        <div className="glass-card text-center p-8">
            ...
            {status === 'error' && (
                <>
                    ...
                    <p className="text-gray-400 mb-6">{message}</p>
                    <Link to="/resend-otp" className="btn-primary block py-3 mb-3">Resend Verification Email</Link>
                    <Link to="/login" className="block text-sm text-gray-400 hover:text-white transition-colors">Back to Login</Link>
                </>
            )}
        </div>
    );
